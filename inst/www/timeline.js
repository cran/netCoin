        ...
        if(!laneData.length){
          ...
          if(options.text)
            ...
            rectsEnter.append("title")
              .text(function(d){ return d[options.name]+" \n"+d[options.start]+((d[options.end]===null)?"":" - "+d[options.end]); })
...
function svg2pdf(){
  ...
  d3.select("div.minor").each(function(){
      var self = d3.select(this),
          txt = self.text(),
          fontsize = parseInt(self.style("font-size")),
          x = margin[0],
